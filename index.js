...
// Answer:  Nested function has access to the internal variable because it is that variable is available in the Myfunction() scope, which nested function is a part of.
...
function USApop(arr) {
  const population = arr.map(element => element.population);
  return population.reduce((accumulator, currentValue) => accumulator + currentValue);
}
